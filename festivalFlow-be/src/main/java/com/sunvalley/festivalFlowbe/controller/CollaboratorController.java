...
public class CollaboratorController {
    ...
    public ResponseEntity<CollaboratorEntity> create(@RequestBody CollaboratorEntity collaborator) {
        if (collaboratorService.phoneIsValid(collaborator.getPhone())) {
            CollaboratorEntity newCollaborator = collaboratorService.create(collaborator);
            emailService.sendCollabortorInfo(newCollaborator.getId());
            return new ResponseEntity<>(newCollaborator, HttpStatus.OK);
        } else {
            return new ResponseEntity<>(HttpStatus.FORBIDDEN);
